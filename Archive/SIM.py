...
# ---- Load .env ----
# Define env_file as a global variable
global env_file
env_file = r"Z:\Survey\UT\_Scripts\SunrisePhoto\resources\.env"
...
def export_gps_and_date_to_csv(renamed_images, client_name, project_name, file_dt, type_str):
    ...
    logging.info(f"RENAMED IMAGES: {renamed_images}")

    first_info = next(iter(renamed_images.values()), None)
    ...
    logging.info("Starting export_gps_and_date_to_csv function")
    logging.info(f"Client Name: {client_name}, Project Name: {project_name}, File Date: {file_dt}, Type String: {type_str}")
    logging.info(f"Initial renamed_images: {renamed_images}")
    ...
    with open(output_file_path, "w", newline="", encoding="utf-8") as csvfile, \
        ...
        logging.info(f"Domain Path: {domain_path}")
        ...
        for info in renamed_images.values():
            ...
            logging.info(f"Generated hyperlink for {base_name}: {hyperlink}")

            if first_hyperlink is None:
                first_hyperlink = hyperlink
                logging.info(f"Set first_hyperlink: {first_hyperlink}")
    ...
    if first_hyperlink is None:
        logging.warning("No hyperlinks were generated. first_hyperlink remains None.")

    logging.info(f"Final first_hyperlink: {first_hyperlink}")
...
def upload_file_via_ftp_with_retry(file_path, remote_dir, max_retries=3, delay_base=2):
    if not Path(file_path).exists():
        logging.warning(f"File not found for FTP: {file_path}")
        return
    
    # Check if FTP credentials are available
    if not all([FTP_SERVER, FTP_USERNAME, FTP_PASSWORD]):
        logging.error(f"FTP credentials missing - Server: {FTP_SERVER}, Username: {FTP_USERNAME}, Password: {'***' if FTP_PASSWORD else 'None'}")
        return
    
    # Debug: Log FTP credentials status
    logging.info(f"FTP_SERVER: {FTP_SERVER}")
    logging.info(f"FTP_USERNAME: {FTP_USERNAME}")
    logging.info(f"FTP_PASSWORD: {'***' if FTP_PASSWORD else 'None'}")
    
    for attempt in range(1, max_retries + 1):
        try:
            logging.info(f"Attempting FTP connection to {FTP_SERVER} (attempt {attempt})")
            with ftplib.FTP(FTP_SERVER, timeout=30) as ftp:
                logging.info(f"FTP connection established, attempting login...")
                ftp.login(FTP_USERNAME, FTP_PASSWORD)
                logging.info(f"FTP login successful, creating remote directory: {remote_dir}")
                create_remote_directory_recursive(ftp, remote_dir)
                logging.info(f"Remote directory created, uploading file: {file_path}")
                with open(file_path, 'rb') as f:
                    ftp.storbinary(f"STOR {Path(file_path).name}", f)
            logging.info(f"FTP uploaded: {file_path}")
            return
        except Exception as e:
...
def upload_files_with_persistent_ftp(files, remote_dir, is_html=True):
    """Upload files using a single persistent FTP connection."""
    if not all([FTP_SERVER, FTP_USERNAME, FTP_PASSWORD]):
        logging.error("FTP credentials are missing.")
        return
            ...
            for file in files:
                try:
                    if is_html:
                        ...
                        file_path = file.get('compressed_path')
...
def upload_html_templates_concurrently(html_files, remote_dir):
    upload_files_with_persistent_ftp(html_files, remote_dir, is_html=True)


def upload_images_concurrently(image_files, remote_dir):
    upload_files_with_persistent_ftp(image_files.values(), remote_dir, is_html=False)
...
def send_html_email(project_name, client_name, date, employee_name, first_link, remote_dir):
    ...
    recipients_file = Path(__file__).parent / "email_recipients.txt"
    ...
    def process_all_files(self):
        ...
        def run_processing():
            ...
            try:
                ...
                logging.info(f"First hyperlink from panos: {first_link}")
                ...
                # Upload HTML files concurrently
                self.status_update_signal.emit("Uploading HTML files...", False)
                upload_html_templates_concurrently(all_html_files, remote_dir)
                processed_steps += num_html
                update_progress()
                # Upload images concurrently
                self.status_update_signal.emit("Uploading images...", False)
                # Combine pano and photo images for concurrent upload
                all_images = {**pano_renamed_images, **photo_renamed_images}
                upload_images_concurrently(all_images, remote_dir)
                processed_steps += num_img
                update_progress()
