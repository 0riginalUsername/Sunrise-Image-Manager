...
- S3 credentials come from encrypted access.json (encrypted using keyring).

S3 Upload Support
-----------------
- Set "upload.use_s3": true in config.json to enable S3 uploads
- Configure "upload.s3_bucket" and "upload.s3_prefix" in config.json
- Store AWS credentials in access.json (will be encrypted using keyring)
- To encrypt existing access.json, call: encrypt_existing_access_json()

Keyring Encryption
-----------------
- AWS credentials are stored securely using keyring
- First time: place credentials in access.json, then call encrypt_existing_access_json()
- Credentials will be moved to keyring, access.json will only contain metadata

Author: ChatGPT (refactor for Kevin)
Date: 2025-09-09
Updated: 2025-01-XX (Added S3 support and keyring encryption)
"""
...
try:
    ...
    logging.warning("keyring not available. Access credentials will not be encrypted.")
...
# S3 Configuration
USE_S3 = cfg_get(CONFIG, "upload.use_s3", False)
S3_BUCKET = cfg_get(CONFIG, "upload.s3_bucket", "")
S3_PREFIX = cfg_get(CONFIG, "upload.s3_prefix", "")
ACCESS_JSON_PATH = Path(cfg_get(CONFIG, "paths.access_json", "./access.json")).expanduser()
KEYRING_SERVICE = cfg_get(CONFIG, "keyring.service_name", "sunrise_photo_manager")
...
# --------------------
# Keyring helpers for encrypted access.json
# --------------------
def encrypt_and_save_access_json(access_data: dict, access_path: Path):
    """
    Encrypt and save access.json using keyring.
    Stores encrypted credentials in keyring, JSON file only contains non-sensitive metadata.
    """
    if not KEYRING_AVAILABLE:
        logging.warning("keyring not available. Saving unencrypted access.json (NOT RECOMMENDED)")
        safe_mkdir(access_path.parent)
        with open(access_path, "w", encoding="utf-8") as f:
            json.dump(access_data, f, indent=2)
        return True
    
    try:
        # Store sensitive credentials in keyring
        keyring.set_password(KEYRING_SERVICE, "aws_access_key", access_data.get("access_key", ""))
        keyring.set_password(KEYRING_SERVICE, "aws_secret_key", access_data.get("secret_key", ""))
        
        # Save non-sensitive metadata to JSON file
        safe_mkdir(access_path.parent)
        metadata = {
            "region": access_data.get("region", "us-east-2"),
            "_encrypted": True,
            "_note": "Credentials stored in keyring service"
        }
        with open(access_path, "w", encoding="utf-8") as f:
            json.dump(metadata, f, indent=2)
        
        logging.info(f"Encrypted access.json saved to {access_path} (credentials in keyring)")
        return True
    except Exception as e:
        logging.error(f"Failed to encrypt access.json: {e}")
        return False

def load_decrypted_access_json(access_path: Path) -> dict | None:
    """
    Load and decrypt access.json using keyring.
    Returns the decrypted JSON data as a dict, or None on failure.
    """
    if not access_path.exists():
        logging.warning(f"access.json not found at {access_path}")
        return None
    ...
    try:
        ...
        if file_data.get("_encrypted") and KEYRING_AVAILABLE:
            ...
            if not access_key or not secret_key:
                logging.error("AWS credentials not found in keyring. Run encrypt_and_save_access_json first.")
                return None
            
            return {
                "access_key": access_key,
                "secret_key": secret_key,
                "region": file_data.get("region", "us-east-2")
            }
        else:
            # Not encrypted or keyring not available, load directly (backward compatibility)
            if not KEYRING_AVAILABLE:
                logging.warning("keyring not available. Loading unencrypted access.json")
            return file_data
    except Exception as e:
        logging.error(f"Failed to load access.json: {e}")
        return None

# --------------------
# S3 Configuration Loading
# --------------------
def load_s3_credentials() -> dict | None:
    """Load S3 credentials from encrypted access.json"""
    access_data = load_decrypted_access_json(ACCESS_JSON_PATH)
    if not access_data:
        return None
    return {
        "access_key": access_data.get("access_key"),
        "secret_key": access_data.get("secret_key"),
        "region": access_data.get("region", "us-east-2")
    }

def encrypt_existing_access_json():
    """
    Utility function to encrypt an existing unencrypted access.json file.
    Call this once to migrate from plain text to encrypted storage.
    """
    if not ACCESS_JSON_PATH.exists():
        logging.error(f"access.json not found at {ACCESS_JSON_PATH}")
        return False
    
    try:
        # Load existing (potentially unencrypted) file
        with open(ACCESS_JSON_PATH, "r", encoding="utf-8") as f:
            existing_data = json.load(f)
        
        # Check if already encrypted
        if existing_data.get("_encrypted") and KEYRING_AVAILABLE:
            logging.info("access.json is already encrypted")
            return True
        
        # Encrypt it
        if encrypt_and_save_access_json(existing_data, ACCESS_JSON_PATH):
            logging.info("Successfully encrypted access.json")
            return True
        else:
            logging.error("Failed to encrypt access.json")
            return False
    except Exception as e:
        logging.error(f"Error encrypting access.json: {e}")
        return False
...
def make_domain_path(client_name, project_name, dt):
    ...
    # Force lowercase for all path components to ensure consistent URLs
    return f"{DOMAIN_PREFIX}/{client_name.lower()}/{project_name.lower()}/{dt.lower()}"
...
def export_gps_and_date_to_csv(renamed_images, client_name, project_name, file_dt, type_str):
    ...
    with open(output_file_path, "w", newline="", encoding="utf-8") as csvfile, \
        ...
        for info in renamed_images.values():
            ...
            # Keep original filename case in hyperlink
...
def render_template(file_name, info, proj_compiled, output_directory, template):
    try:
        ...
        dt_obj = datetime.now()
    converted_dt = dt_obj.strftime("%d-%b-%y %I:%M%p")
...
# --------------------
def get_s3_client():
    """Get a configured S3 client using credentials from encrypted access.json"""
    if not BOTO3_AVAILABLE:
        logging.error("boto3 not available. Cannot create S3 client.")
        return None
    
    creds = load_s3_credentials()
    if not creds or not creds.get("access_key") or not creds.get("secret_key"):
        logging.error("S3 credentials not found in access.json")
        return None
...
def upload_file_to_s3_with_retry(file_path, remote_dir, max_retries=3, delay_base=2):
    """
    Upload a file to S3 bucket, with optional retry support and exponential backoff.
    remote_dir is used as the S3 key prefix.
    The S3 key should match the domain path structure for proper URL mapping.
    """
    if not USE_S3 or not S3_BUCKET:
        logging.warning("S3 upload disabled or bucket not configured")
        return
    ...
    # Construct S3 key to match domain path structure
    # Domain path is: /auto/client/project/dt/filename
    # S3 key should be: auto/client/project/dt/filename (no leading slash)
    # Keep path lowercase but preserve original filename case
    filename = os.path.basename(file_path)
    
    # Use domain prefix structure to match the URL path
    # Remove leading/trailing slashes and construct path
    prefix = S3_PREFIX.strip('/').lower() if S3_PREFIX else ""
    remote = remote_dir.strip('/').lower()
    # Keep original filename case (don't force lowercase)
    
    if prefix:
        s3_key = f"{prefix}/{remote}/{filename}"
    else:
        s3_key = f"{remote}/{filename}"
    
    # Ensure no leading slash (S3 keys shouldn't start with /)
    s3_key = s3_key.lstrip('/')
    
    # Determine Content-Type based on file extension (use lowercase for detection)
    ...
    # Set metadata for proper browser handling
    # Note: ACL will be tried first, but if bucket doesn't allow ACLs, we'll retry without it
    # Your bucket policy already handles public access, so ACL is optional
    extra_args = {
        'ContentType': content_type,
        'CacheControl': 'max-age=3600',  # Cache for 1 hour
        'ACL': 'public-read'  # Try ACL first, will retry without if bucket doesn't support it
    }
    
    # Note: CORS must be configured at the bucket level in AWS S3 Console
    # Required CORS configuration for the bucket:
    # [
    #   {
    #     "AllowedHeaders": ["*"],
    ...
    # Track if we've already tried without ACL (to avoid retrying with ACL)
    acl_removed = False
    
    for attempt in range(1, max_retries + 1):
        try:
            logging.info(f"Starting S3 upload for: {file_path} -> s3://{S3_BUCKET}/{s3_key} (Content-Type: {content_type}) (Attempt {attempt})")
            
            # Upload file with proper Content-Type
            # If ACL fails, retry without ACL (bucket policy should handle public access)
            try:
                with open(file_path, 'rb') as f:
                    s3_client.upload_fileobj(f, S3_BUCKET, s3_key, ExtraArgs=extra_args)
            except ClientError as acl_error:
                error_code = acl_error.response.get('Error', {}).get('Code', 'Unknown')
                if error_code in ['AccessDenied', 'AccessControlListNotSupported'] and 'ACL' in extra_args and not acl_removed:
                    # Remove ACL and retry - bucket policy should handle public access
                    # Some buckets don't allow ACLs (Object Ownership settings)
                    logging.warning(f"ACL setting failed ({error_code}), retrying without ACL for {filename}")
                    extra_args = {k: v for k, v in extra_args.items() if k != 'ACL'}
                    acl_removed = True
                    with open(file_path, 'rb') as f:
                        s3_client.upload_fileobj(f, S3_BUCKET, s3_key, ExtraArgs=extra_args)
                    logging.info(f"Upload succeeded without ACL (using bucket policy for access)")
                else:
                    raise  # Re-raise if it's a different error or we've already removed ACL
            
            # Verify upload succeeded by checking if object exists
            try:
                s3_client.head_object(Bucket=S3_BUCKET, Key=s3_key)
                logging.info(f"[SUCCESS] Successfully uploaded and verified: {filename} -> s3://{S3_BUCKET}/{s3_key}")
            except ClientError as verify_error:
                logging.warning(f"Upload appeared successful but verification failed: {verify_error}")
                # Continue anyway as upload might have succeeded
            
            return  # Success, exit the loop
        except ClientError as e:
            error_code = e.response.get('Error', {}).get('Code', 'Unknown')
            error_msg = e.response.get('Error', {}).get('Message', str(e))
            logging.error(f"[ERROR] S3 upload attempt {attempt} failed for {file_path}: {error_code} - {error_msg}")
            logging.error(f"   S3 Key: s3://{S3_BUCKET}/{s3_key}")
            if attempt == max_retries:
                logging.error(f"[ERROR] Giving up on {file_path} after {max_retries} attempts. File may not be accessible.")
            else:
                sleep_time = delay_base ** attempt
                logging.info(f"Retrying in {sleep_time} seconds...")
                time.sleep(sleep_time)
        except Exception as e:
            logging.error(f"[ERROR] S3 upload attempt {attempt} failed for {file_path}: {type(e).__name__}: {e}")
            logging.error(f"   S3 Key: s3://{S3_BUCKET}/{s3_key}")
            if attempt == max_retries:
                logging.error(f"[ERROR] Giving up on {file_path} after {max_retries} attempts. File may not be accessible.")
            else:
...
def upload_files_to_s3_concurrently(files, remote_dir):
    """
    ...
    max_threads = int(cfg_get(CONFIG, "upload.s3_max_threads", 6))
    
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        for file_path in files:
            executor.submit(upload_file_to_s3_with_retry, file_path, remote_dir)
...
def upload_file_via_ftp_with_retry(file_path, remote_dir, max_retries=3, delay_base=2):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            logging.info(f"Starting FTP upload for: {file_path} (Attempt {attempt})")
            with ftplib.FTP(FTP_SERVER, timeout=30) as ftp:
                ftp.login(FTP_USERNAME, FTP_PASSWORD)
                create_remote_directory_recursive(ftp, remote_dir)
                with open(file_path, 'rb') as f:
                    ftp.storbinary(f"STOR {os.path.basename(file_path)}", f)
            logging.info(f"Successfully uploaded {os.path.basename(file_path)} via FTP on attempt {attempt}")
            return  # Success, exit the loop
        except Exception as e:
            logging.warning(f"FTP attempt {attempt} failed for {file_path}: {e}")
            if attempt == max_retries:
...
def upload_file_universal(file_path, remote_dir, max_retries=3, delay_base=2):
    """
    Universal upload function that routes to S3 or FTP based on configuration.
    """
    if USE_S3:
        upload_file_to_s3_with_retry(file_path, remote_dir, max_retries, delay_base)
    else:
        upload_file_via_ftp_with_retry(file_path, remote_dir, max_retries, delay_base)
...
def upload_html_templates_concurrently(html_files, remote_dir):
    """
    Upload HTML templates concurrently (S3 or FTP based on configuration).
    """
    ...
    if USE_S3:
        # Upload to S3
        upload_files_to_s3_concurrently(html_files, remote_dir)
    else:
        # Upload via FTP
...
def upload_images_concurrently(image_files, remote_dir):
    """
    Upload images concurrently using dynamic thread management (S3 or FTP based on config).
    """
    if USE_S3:
        # Upload to S3
        file_paths = [info['compressed_path'] for info in image_files.values() if info.get('compressed_path')]
        upload_files_to_s3_concurrently(file_paths, remote_dir)
    else:
        # Upload via FTP
        max_threads = probe_ftp_max_threads()
        with ThreadPoolExecutor(max_workers=max_threads) as executor:
...
def send_html_email(project_name, client_name, date, employee_name, first_link, remote_dir):
    """
    Send HTML email via SMTP using credentials from .env file.
    This function uses SMTP (Simple Mail Transfer Protocol) for email delivery.
    """
    ...
    html_content = template.render(
        ...
        PANO_LINK=str(first_link) if first_link else "",
    ...
    try:
        # Use SMTP for email delivery (credentials from .env)
        with smtplib.SMTP(EMAIL_HOST, EMAIL_PORT) as smtp:
            smtp.starttls()  # Enable TLS encryption
            smtp.login(EMAIL_USER, EMAIL_PASS)
            smtp.send_message(msg)
        logging.info("Status email sent successfully via SMTP.")
    except Exception as e:
...
class Pano2DWGApp(QMainWindow):
    ...
    def process_all_files(self):
        ...
        # Force lowercase for remote_dir to match URL structure
        remote_dir = f"{client_name.lower()}/{project_name.lower()}/{dt.lower()}"
        ...
        def run_processing():
            ...
            try:
                ...
                # If no pano link, get first link from photos
                if first_link is None:
                    first_link = export_gps_and_date_to_csv(photo_renamed_images, client_name, project_name, dt, "photo")
                else:
                    # Still export photos CSV even if we got pano link
                ...
                upload_method = "S3" if USE_S3 else "FTP"
                self.status_update_signal.emit(f"Uploading HTML files via {upload_method}...", False)
                failed_uploads = []
                for html in all_html_files:
                    try:
                        upload_file_universal(html, remote_dir)
                        # Small delay to ensure upload completes
                        time.sleep(0.1)
                    except Exception as e:
                        logging.error(f"Failed to upload HTML file {html}: {e}")
                        failed_uploads.append(html)
                    processed_steps += 1
                    update_progress()
                if failed_uploads:
                    logging.warning(f"[WARNING] {len(failed_uploads)} HTML file(s) failed to upload. Check logs for details.")
                # Upload images with progress
                self.status_update_signal.emit(f"Uploading images via {upload_method}...", False)
                for info in pano_renamed_images.values():
                    if info.get('compressed_path'):
                        upload_file_universal(info['compressed_path'], remote_dir)
                        processed_steps += 1
                        update_progress()
...
if __name__ == "__main__":
    # Check if access.json needs encryption (informational only)
    if ACCESS_JSON_PATH.exists() and KEYRING_AVAILABLE:
        try:
            with open(ACCESS_JSON_PATH, "r", encoding="utf-8") as f:
                data = json.load(f)
                if not data.get("_encrypted") and data.get("access_key"):
                    logging.info("[WARNING] access.json contains unencrypted credentials. "
                               "Consider calling encrypt_existing_access_json() to secure them.")
        except Exception:
            pass  # Ignore errors during startup check
    
    # Check S3 configuration if enabled
    if USE_S3:
        if not BOTO3_AVAILABLE:
            logging.error("[ERROR] S3 upload is enabled but boto3 is not installed. "
                        "Install with: pip install boto3")
        elif not S3_BUCKET:
            logging.error("[ERROR] S3 upload is enabled but s3_bucket is not configured in config.json")
        else:
            creds = load_s3_credentials()
            if not creds:
                logging.warning("[WARNING] S3 credentials not found. Uploads will fail.")
            else:
                logging.info("[SUCCESS] S3 upload configured and ready")
